...
def main():
    if len(sys.argv) < 4:
        ...
        df['listPrice'] = df['listPrice'].astype(str)
        df['discountPrice'] = df['discountPrice'].astype(str)
        df['discountPriceCommerce'] = df['discountPriceCommerce'].astype(str)
        df['totalPrice'] = df['totalPrice'].astype(str)
        df['starScore'] = df['starScore'].astype(str)
        ...
        df.to_excel(fileadress, sheet_name='가격순위', index=False)  # index=False로 설정하여 인덱스를 저장하지 않습니다.
        print(f"데이터프레임이 {fileadress} 파일로 저장되었습니다.")
        # wb = load_workbook(fileadress)
        # writer: ExcelWriter[Any] | None
        # with pd.ExcelWriter(fileadress, mode='a', engine='openpyxl') as writer:
        #     writer.book = wb
        #     df.to_excel(writer, sheet_name='가격순위', index=False)
